...
class Trainer(BaseTrainer):
    ...
    Trainer class
    """
    def __init__(self, model, criterion_1, criterion_2, metric_ftns, optimizer, config, data_loader, fold_id,
                 valid_data_loader=None, class_weights=None):
        super().__init__(model, criterion_1, criterion_2, metric_ftns, optimizer, config, fold_id)
        self.config = config
    ...
    def _train_epoch(self, epoch, total_epochs):
        ...
        for batch_idx, (data, target) in enumerate(self.data_loader):
            ...
            output, vif = self.model(data)

            batch_size, channel_size, node_size = vif.size()
            vif_target = torch.ones(batch_size * channel_size * node_size) * 0.5
            vif_target = vif_target.reshape(-1, channel_size, node_size)
            vif_target = vif_target.to("cuda")

            loss_1 = self.criterion_1(vif, vif_target)
            loss_2 = self.criterion_2(output, target, self.class_weights)
            loss = loss_1 + loss_2
